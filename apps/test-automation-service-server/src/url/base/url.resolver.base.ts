...
export class UrlResolverBase {
  ...
  async createUrl(@graphql.Args() args: CreateUrlArgs): Promise<Url> {
    return await this.service.createUrl({
      ...
      data: {
        ...
        sitemap: args.data.sitemap
          ? {
              connect: args.data.sitemap,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => Sitemap, {
    nullable: true,
    name: "sitemap",
  })
  async getSitemap(@graphql.Parent() parent: Url): Promise<Sitemap | null> {
    const result = await this.service.getSitemap(parent.id);
